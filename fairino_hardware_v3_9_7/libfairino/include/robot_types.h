...
typedef struct _EXT_AXIS_STATUS
{
	...
	uint8_t ready;        //伺服准备好  0-未使能；1-使能
	uint8_t inPos;        //伺服到位
	uint8_t alarm;        //伺服报警
	uint8_t flerr;        //跟随误差
	uint8_t nlimit;       //到负限位
...
typedef struct _ROBOT_STATE_PKG
{
	...
	uint8_t  gripper_motiondone; // 夹爪运动完成信号，0-未完成，1-运动完成(检测到物体)，2-运动完成(未检测到物体) 
	...
	uint16_t gripper_fault;      // 夹爪故障；0-无故障；1-485超时；2-指令错误；3-工件掉落；其它：夹爪故障码+3
...
enum class RobotState
{
	...
	GripperMotiondone = 33,     // 夹爪运动完成信号，0-未完成，1-运动完成(检测到物体)，2-运动完成(未检测到物体) 
	...
	GripperFault = 40,          // 夹爪故障；0-无故障；1-485超时；2-指令错误；3-工件掉落；其它：夹爪故障码+3
